...
public class BoardSettingsDrawer : PropertyDrawer
{
    ...
    override public void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        ...
        SerializedProperty startingObjects = property.FindPropertyRelative("startingObjects");
        startingObjects.arraySize = width;
        float sX = position.x;
        float sY = position.y + (18f * 2);
        float rWidth = position.width / width;
        for (int i = 0; i < width; i++) {
            SerializedProperty rowData = startingObjects.GetArrayElementAtIndex(i).FindPropertyRelative("rows");
